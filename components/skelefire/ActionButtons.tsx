...
export default function ActionButtons() {
  ...
  const { activities, setActivities, countUnreadActivities, markAsRead } =
    useSkelefire()
  ...
  async function refreshSkelefire() {
    ...
    if (unreadActivities.data.length > 0)
      Push.create('Skelefire', {
        ...
        icon: process.env.BACKEND_URL + '/kokoa-png',
        onClick: function () {
          window.focus()
          this.close()
        },
  ...
  function markAllAsRead() {
    const moduleIDs = activities.map((activity) => activity.moduleID)
    markAsRead(moduleIDs)
  }
